...
if ! which "$INTERFACES" >/dev/null; then
    # loop until interface is found, or we give up
    NEXT_WAIT_TIME=1
    until [ -e "/sys/class/net/$INTERFACES" ] || [ $NEXT_WAIT_TIME -eq 4 ]; do
        sleep $(( NEXT_WAIT_TIME++ ))
        echo "Waiting for interface '$INTERFACES' to become available... ${NEXT_WAIT_TIME}"
    done
    if [ -e "/sys/class/net/$INTERFACES" ]; then
        IFACE="$INTERFACES"
    fi
fi
